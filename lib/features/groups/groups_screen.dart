          ...
          child: Column(
            ...
            children: [
              ...
              groupsAsync.when(
                ...
                data: (groups) {
                  if (groups.isEmpty) return _EmptyState(onNew: () => context.push('/groups/new'));
                  return Column(
                    children: [
                            ...
                            child: Row(
                              children: [
                                ...
                                if (g.meetingTime != null)
                                  Text(g.meetingTime!, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: c.faint)),
                              ],
                            ),
                          ),
                        ),
    ...
    showModalBottomSheet(
      ...
      builder: (sheetContext) => Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(group.name, style: PgText.serif(size: 21, weight: FontWeight.w600)),
            const SizedBox(height: 4),
            Text('${group.memberCount} member${group.memberCount == 1 ? '' : 's'}${group.meetingTime != null ? ' · ${group.meetingTime}' : ''}',
                style: TextStyle(fontSize: 13, color: c.dim)),
            if (group.inviteCode != null) ...[
              const SizedBox(height: 18),
              Text('INVITE CODE', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 1, color: c.dim)),
              const SizedBox(height: 8),
              Container(
                padding: const EdgeInsets.fromLTRB(16, 12, 6, 12),
                decoration: BoxDecoration(color: c.surface2, border: Border.all(color: c.line), borderRadius: BorderRadius.circular(14)),
                child: Row(
                  children: [
                    Expanded(
                      child: Text(group.inviteCode!,
                          style: TextStyle(fontSize: 14.5, color: c.text, fontWeight: FontWeight.w700, fontFamily: 'monospace')),
                    ),
                    TextButton(
                      onPressed: () {
                        Clipboard.setData(ClipboardData(text: group.inviteCode!));
                        ScaffoldMessenger.of(sheetContext).showSnackBar(const SnackBar(content: Text('Invite code copied')));
                      },
                      child: Text('Copy', style: TextStyle(color: c.teal, fontSize: 13, fontWeight: FontWeight.w800)),
                    ),
                  ],
                ),
              ),
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Text('No groups yet', style: PgText.serif(size: 21, weight: FontWeight.w600)),
          const SizedBox(height: 8),
          SizedBox(
            width: 260,
            child: Text(
          ...
          const SizedBox(height: 16),
          PgButton(label: 'Start or join a group', expand: false, onPressed: onNew),
        ],
      ),
    );
  }
