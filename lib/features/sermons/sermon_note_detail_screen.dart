...
class _NoteDetailState extends ConsumerState<_NoteDetail> {
  ...
  late final _speakerCtrl = TextEditingController(text: widget.note.speaker ?? '');
  late final _scriptureCtrl = TextEditingController(text: widget.note.scriptureRef ?? '');
  ...
  Future<void> _togglePlay(SermonRecording recording) async {
    ...
    try {
      final url = await ref.read(sermonNotesRepositoryProvider).signedAudioUrl(recording.audioPath);
  ...
  Future<void> _stopAndAttach() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not save recording: $e')));
      }
    }
  }
  ...
  Future<void> _deleteRecording(SermonRecording recording) async {
    ...
    try {
      await ref.read(sermonNotesProvider.notifier).deleteRecording(noteId: widget.note.id, recording: recording);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not delete recording: $e')));
      }
    }
  }
  ...
  Future<void> _saveEdit() async {
    ...
    if (title.isEmpty) {
      setState(() => _editError = 'Give this sermon a title.');
      return;
    }
    setState(() {
      _savingEdit = true;
    ...
    try {
      await ref.read(sermonNotesProvider.notifier).updateNote(
            ...
            scriptureRef: _scriptureCtrl.text.trim().isEmpty ? null : _scriptureCtrl.text.trim(),
  ...
  Future<void> _delete() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('This removes the note and its recordings, if any. This can\'t be undone.'),
        actions: [
          TextButton(onPressed: () => Navigator.of(context).pop(false), child: const Text('Cancel')),
          TextButton(onPressed: () => Navigator.of(context).pop(true), child: const Text('Delete')),
        ],
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        PgHeader(
          onBack: _editing ? () => setState(() => _editing = false) : () => context.pop(),
          trailing: _editing
              ? TextButton(
              ...
              : Row(
                  ...
                  children: [
                    IconButton(onPressed: _startEdit, icon: Icon(Icons.edit_outlined, color: c.dim)),
                    IconButton(
        ...
        Expanded(
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                if (_editing) ...[
                  PgTextField(controller: _titleCtrl, hint: 'Sermon title', fontWeight: FontWeight.w700),
                  const SizedBox(height: 10),
                  Row(
                    children: [
                      Expanded(child: PgTextField(controller: _speakerCtrl, hint: 'Speaker (optional)')),
                      const SizedBox(width: 10),
                      Expanded(child: PgTextField(controller: _scriptureCtrl, hint: 'Scripture ref (optional)')),
                    ],
                  ),
                  if (_editError != null) ...[
                    const SizedBox(height: 8),
                    Text(_editError!, style: TextStyle(color: c.danger, fontSize: 12.5)),
                  ],
                ] else ...[
                  ...
                  for (var i = 0; i < note.recordings.length; i++)
                    Padding(
                      ...
                      child: _RecordingRow(
                        ...
                        playing: _playingRecordingId == note.recordings[i].id && _playing,
                ...
                if (_canRecord) ...[
                  ...
                  if (_recordingNew)
                    ...
                    OutlinedButton.icon(
                      ...
                      label: Text('Record a new one', style: TextStyle(color: c.teal, fontWeight: FontWeight.w700)),
                    ),
                  if (_recordError != null) ...[
                    const SizedBox(height: 8),
                    Text(_recordError!, style: TextStyle(color: c.danger, fontSize: 12.5)),
                  ],
                ],
        ...
        children: [
          Material(
            ...
            child: InkWell(
              ...
              child: SizedBox(
                ...
                height: 42,
                child: showLoading
                    ? Padding(padding: const EdgeInsets.all(11), child: CircularProgressIndicator(strokeWidth: 2, color: c.onTeal))
                    : Icon(playing ? Icons.pause_rounded : Icons.play_arrow_rounded, color: c.onTeal),
              ),
            ),
          ),
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Take $index', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: c.dim)),
                Text(
                  isActive && duration > Duration.zero
                      ? '${fmt(position)} / ${fmt(duration)}'
                      : (recording.durationSeconds != null ? fmt(Duration(seconds: recording.durationSeconds!)) : 'Recording'),
          ...
          InkWell(
            ...
            child: Padding(
              ...
              child: Icon(Icons.delete_outline_rounded, size: 18, color: c.faint),
            ),
          ),
        ],
      ),
        ...
        children: [
          ...
          Material(
            ...
            child: InkWell(
              ...
              child: SizedBox(
                width: 40,
                height: 40,
                child: Icon(paused ? Icons.mic_rounded : Icons.pause_rounded, color: c.text, size: 18),
              ),
            ),
          ),
          const SizedBox(width: 10),
          Material(
            ...
            child: InkWell(
              customBorder: const CircleBorder(),
              onTap: onStop,
              child: const SizedBox(width: 40, height: 40, child: Icon(Icons.stop_rounded, color: Colors.white, size: 18)),
            ),
          ),
        ],
      ),
