    ...
    return Scaffold(
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            PgHeader(title: 'Focus Mode', onBack: () => context.pop()),
            Padding(
            ...
            Row(
              ...
              children: [
                const PgSectionLabel('Apps to quiet', padding: EdgeInsets.zero),
                Text('Social preset', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: c.teal)),
              ],
            ),
                  ...
                  for (var i = 0; i < focusApps.length; i++)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        border: i == focusApps.length - 1 ? null : Border(bottom: BorderSide(color: c.line)),
                      ),
                      child: Row(
                        children: [
                          Container(
                            width: 34,
                            height: 34,
                            decoration: BoxDecoration(color: focusApps[i].$2, borderRadius: BorderRadius.circular(9)),
                          ),
                          const SizedBox(width: 12),
                          Expanded(child: Text(focusApps[i].$1, style: const TextStyle(fontSize: 14.5, fontWeight: FontWeight.w600))),
                          _MiniSwitch(on: i < 3),
                        ],
                      ),
                    ),
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: c.bg,
                  border: Border.all(color: active ? c.teal : c.line2, width: active ? 6 : 1.5),
                ),
              ),
              ...
              if (trailingBg != null)
                Container(
                  ...
                  decoration: BoxDecoration(color: trailingBg, borderRadius: BorderRadius.circular(100)),
                  child: Text(trailing, style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w800, color: trailingColor)),
                )
              else
                Text(trailing, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: trailingColor)),
            ],
          ),
        ),
      ),
...
class _MiniSwitch extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: on ? c.teal : c.line2, borderRadius: BorderRadius.circular(100)),
      child: Align(
        alignment: on ? Alignment.centerRight : Alignment.centerLeft,
        child: Container(width: 20, height: 20, decoration: BoxDecoration(shape: BoxShape.circle, color: on ? Colors.white : c.dim)),
      ),
    );
  }
}
