                    ...
                    Wrap(
                      ...
                      children: [
                        for (final l in const [7, 21, 30, 40])
                          PgPill(label: '$l days', active: _length == l, onTap: () => setState(() => _length = l)),
                        PgPill(label: 'Custom', active: ![7, 21, 30, 40].contains(_length), onTap: () => setState(() => _length = 100)),
                      ],
                    ),
                    const SizedBox(height: 22),
                    Text('FOCUS', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, letterSpacing: 1, color: c.dim)),
                    const SizedBox(height: 10),
                    Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: [
                        for (final f in const ['Growth', 'Fasting', 'Revival', 'Thanksgiving'])
                          PgPill(label: f, active: _focus == f, onTap: () => setState(() => _focus = f)),
                      ],
                    ),
                    const SizedBox(height: 22),
                    Text('DAILY REMINDER', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, letterSpacing: 1, color: c.dim)),
                    const SizedBox(height: 10),
                    Container(
                      ...
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          const Text('8:00 AM', style: TextStyle(fontSize: 14.5, fontWeight: FontWeight.w600)),
                          ...
                          child: Row(
                            children: [
                              Container(
                                ...
                                child: Icon(Icons.person_add_alt_1_outlined, size: 20, color: c.teal),
                              ),
                              ...
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    const Text('Invite a companion', style: TextStyle(fontSize: 14.5, fontWeight: FontWeight.w700)),
                                    Text('Take it on together and cheer each other on', style: TextStyle(fontSize: 12, color: c.dim)),
                                  ],
                                ),
                              ),
