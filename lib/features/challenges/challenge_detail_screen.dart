      ...
      body: SafeArea(
        ...
        child: SingleChildScrollView(
          ...
          child: challengesAsync.when(
            ...
            data: (_) {
              final progress = ref.read(challengeProvider.notifier).forKey(challengeKey);
              ...
              if (progress == null && catalogEntry == null) {
                return Padding(
                  ...
                  child: Column(
                    children: [
                      Text("Challenge not found", style: PgText.serif(size: 20, weight: FontWeight.w600)),
                      const SizedBox(height: 16),
                      PgButton(label: 'Back to Challenges', expand: false, onPressed: () => context.pop()),
                    ],
                  ),
                );
              }
              ...
              final totalDays = progress?.totalDays ?? catalogEntry!.lengthDays;
              final currentDay = progress?.currentDay ?? 0;
              final description = catalogEntry?.description ?? 'A personal challenge — stay consistent and keep showing up.';
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Text('$totalDays days', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: c.dim)),
                    ],
                  ),
                  const SizedBox(height: 14),
                  Text(name, style: PgText.serif(size: 29, weight: FontWeight.w600)),
                  const SizedBox(height: 6),
                  Padding(
                    padding: const EdgeInsets.only(bottom: 22),
                    child: Text(description, style: TextStyle(fontSize: 14.5, height: 1.6, color: c.dim)),
                  ),
                  Row(
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            finished ? 'Every day, complete.' : (focusToday ?? 'Day ${currentDay + 1} of $totalDays'),
                  ...
                  if (!finished)
                    PgButton(
                      label: currentDay > 0 ? 'Continue — Day ${currentDay + 1}' : 'Start challenge',
                      onPressed: () async {
                        await ref.read(challengeProvider.notifier).startOrAdvance(
                    ...
                    PgButton(label: 'Challenge complete', onPressed: () => context.pop()),
