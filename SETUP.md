...
clones the repo, before it builds — it installs Flutter (a shallow clone
of the `stable` channel, since Xcode Cloud's VM starts empty every run),
runs `flutter pub get`, and runs `pod install`. This is required: without
it, `xcodebuild` fails immediately with `Could not resolve package
dependencies ... FlutterGeneratedPluginSwiftPackage ... doesn't exist in
file system` — Xcode Cloud only checks out git, and that Swift Package is
generated by `flutter pub get`, not committed. No setup needed beyond the
script already being in the repo — Xcode Cloud finds and runs it by
convention (the exact path), as long as its "Executable" file permission
bit is preserved (`git update-index --chmod=+x` if it's ever lost).
